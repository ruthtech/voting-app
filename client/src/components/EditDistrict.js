...
function EditDistrict() {
  ...
  const handleFormSubmit = async (user, event) => {
    try {
      ...
      await axios.put(`/api/updateAddress/${user.uuid}/${address}/${city}/${province}/${newDistrict}`);

      return <EditDistrictConfirm user={user}/>
    } catch( err ) {
      console.log(err);
                    ...
                    <div className="col">
                      <Form>
                          ...
                          <div className="right-align-div">
                            <Button variant="secondary" type="submit" onClick={(event) => {handleFormSubmit(user, event)}}>
                              Save
                            </Button>
                          </div>
                        </Form>
